...
} else if (!isset($_GET['fn']) || $_GET['fn'] == "") { // デフォルトはログイン画面
    require($php_path . DIRECTORY_SEPARATOR . "gz_logon.php");
} else if (in_array("{$_GET['fn']}.php", $phpfilelist)) { // 存在するページ(.php)のみ通す
    if ($_GET['fn'] == "gz_logon") {
        require($php_path . DIRECTORY_SEPARATOR . "gz_logon.php");
    } else if ($_GET['fn'] == "gz_logon2") {
        require($php_path . DIRECTORY_SEPARATOR . "{$_GET['fn']}.php");
